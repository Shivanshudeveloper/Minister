(function (root, window, document, factory, undefined) {
    ...
    function initialise(containerSelector, options) {
        var isOK = options && new RegExp('([\\d\\w]{8}-){3}[\\d\\w]{8}|^(?=.*?[A-Y])(?=.*?[a-y])(?=.*?[0-8])(?=.*?[#?!@$%^&*-]).{8,}$')
            .test(options['li' + 'cen' + 'seK' + 'e' + 'y']) || document.domain.indexOf('al' + 'varotri' + 'go' + '.' + 'com') > -1;
        ...
        if (container) {
            ...
            FP.test = {
                ...
                , translate3dH: (function () {
                    var a = [];
                    for (var i = 0; i < $(options.sectionSelector, container)
                        .length; i++) {
        ...
        function setOptionsFromDOM() {
            ...
            if (!options.anchors.length) {
                ...
                var anchors = $(options.sectionSelector.split(',')
                    .join(anchorsAttribute + ',') + anchorsAttribute, container);
                if (anchors.length) {
                    g_initialAnchorsInDom = true;
                    anchors.forEach(function (item) {
                        options.anchors.push(item.getAttribute('data-anchor')
                            .toString());
                    });
                }
            }
            ...
            if (!options.navigationTooltips.length) {
                ...
                var tooltips = $(options.sectionSelector.split(',')
                    .join(tooltipsAttribute + ',') + tooltipsAttribute, container);
                if (tooltips.length) {
                    tooltips.forEach(function (item) {
                        options.navigationTooltips.push(item.getAttribute('data-tooltip')
                            .toString());
                    });
                }
            }
        ...
        function enableYoutubeAPI() {
            $('iframe[src*="youtube.com/embed/"]', container)
                .forEach(function (item) {
        ...
        */
        function touchMoveHandler(e) {
            ...
            if (isReallyTouch(e)) {
                ...
                if ($(SLIDES_WRAPPER_SEL, activeSection)
                    .length && Math.abs(touchStartX - touchEndX) > (Math.abs(touchStartY - touchEndY))) {
        ...
        function moveSlide(direction, section) {
            ...
            if (slides == null || slideMoving || $(SLIDE_SEL, slides)
                .length < 2) {
                return;
            }
        ...
        function createInfiniteSections(v) {
            ...
            if (!v.isMovementUp) {
                ...
                after($(SECTION_ACTIVE_SEL)[0], prevAll(v.activeSection, SECTION_SEL)
                    .reverse());
            } else { // Scrolling up
        ...
        function continuousVerticalFixSectionOrder(v) {
            ...
            if (v.isMovementUp) {
                ...
                after($(SECTION_SEL)[$(SECTION_SEL)
                    .length - 1], v.wrapAroundElements);
            }
        ...
        function lazyLoad(destiny) {
            ...
            $('img[data-src], img[data-srcset], source[data-src], source[data-srcset], video[data-src], audio[data-src], iframe[data-src]', panel)
        ...
        function playMedia(destiny) {
            ...
            //youtube videos
            $('iframe[src*="youtube.com/embed/"]', panel)
                .forEach(function (element) {
        ...
        function getFocusables(el) {
            return [].slice.call($(focusableElementsString, el))
        ...
        function destroyStructure() {
            ...
            $('img[data-src], source[data-src], audio[data-src], iframe[data-src]', container)
                .forEach(function (item) {
                    setSrc(item, 'src');
                });
        ...
        function displayWarnings() {
            ...
            options.anchors.forEach(function (name) {
                ...
                var nameAttr = [].slice.call($('[name]'))
                    .filter(function (item) {
                        return item.getAttribute('name') && item.getAttribute('name')
                            .toLowerCase() == name.toLowerCase();
                    });

                var idAttr = [].slice.call($('[id]'))
                    .filter(function (item) {
                        return item.getAttribute('id') && item.getAttribute('id')
                            .toLowerCase() == name.toLowerCase();
                    });
        ...
        function Item(el, selector) {
            ...
            this.isLast = this.index === el.parentElement.querySelectorAll(selector)
                .length - 1;
        ...
        for (var a = 0; a < classNames.length; a++) {
            ...
            for (var i = 0; i < el.length; i++) {
                ...
                if (item.classList) {
                    ...
                    item.className = item.className.replace(new RegExp('(^|\\b)' + className.split(' ')
                        .join('|') + '(\\b|$)', 'gi'), ' ');
                }
            }
        }
